...
<div class="mx-auto max-w-3xl">
    ...
    <div class="flex items-center gap-3">
        <h1 class="text-2xl font-bold tracking-tight text-zinc-900">Create Doc</h1>
    </div>
    ...
    <div class="mt-6 overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm">
        <form action="{{ route('dashboard.docs.store') }}" method="post" enctype="multipart/form-data" class="p-6 sm:p-8">
            @csrf
            <div class="space-y-6">
                <div>
                    <label for="title" class="block text-sm font-medium text-zinc-700">Title</label>
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required minlength="5" maxlength="160" class="mt-2 block h-11 w-full rounded-xl border border-zinc-200 bg-white px-4 text-zinc-900 shadow-sm transition placeholder:text-zinc-400 focus:border-zinc-300 focus:outline-none focus:ring-4 focus:ring-zinc-100" placeholder="Enter document title…" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="content" type="hidden" name="content" value="{{ old('content', '') }}" required minlength="30">
